...
class Register extends Worker
{
    ...
    public function onMessage($connection, $buffer)
    {
        ...
        if (empty($data['event'])) {
            $error = "Bad request for Register service. If you are a client please connect Gateway. Request info(IP:".$connection->getRemoteIp().", Request Buff:$buffer)\n";
            echo $error;
        ...
        switch ($event) {
            ...
            case 'ping':
                break;
            default:
                echo "unknown event:$event IP: ".$connection->getRemoteIp()." Buff:$buffer\n";
                $connection->close();
        }
    }
